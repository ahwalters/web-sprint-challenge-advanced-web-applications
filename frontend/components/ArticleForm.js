...
export default function ArticleForm(props) {
  ...
  useEffect(() => {
    ...
    if (currentArticleId) {
      const currentArticle = articles.filter((el) => el.article_id === currentArticleId)
  ...
  const isDisabled = () => {
    ...
    if (values.title.trim().length >=1 && values.text.trim().length >=1 && values.topic !== '-- Select topic --'){
      return false
    } else return true;
  }
